...
void readFileSizeAndContent(string filename, int& size, string& content) {
    ...
    if (file.is_open()) {
        size = file.tellg();
        cout<<"size of file "<<filename<<" is "<<size<<endl;
        // gives the size of file in bytes
        file.seekg(0, ios::beg); // reset the file pointer back to 0
...
void printSymbolTable(const SymbolTable& symbolTable, int startAddress) {
    ...
    for (const auto& variable : symbolTable.variables) {
        string nameWithoutSemicolon = variable.name;
        if (!nameWithoutSemicolon.empty() && nameWithoutSemicolon.back() == ';') {
            nameWithoutSemicolon.pop_back();
            Variable modifiedVariable = variable; // Create a copy of the variable
            modifiedVariable.name = nameWithoutSemicolon; // Modify the name of the copied variable
            cout << modifiedVariable.name << "\t\t" << modifiedVariable.type << "\t" << modifiedVariable.size << "\t" << modifiedVariable.address << endl;
        } else {
...
void printSymbolTablet(const SymbolTable& symbolTable) {
    ...
    for (const auto& variable : symbolTable.variables) {
        ...
        if (!nameWithoutSemicolon.empty() && nameWithoutSemicolon.back() == ';') {
            ...
            Variable modifiedVariable = variable; // Create a copy of the variable
            modifiedVariable.name = nameWithoutSemicolon; // Modify the name of the copied variable
            cout << modifiedVariable.name << "\t" << modifiedVariable.type << endl;
        }
        else
